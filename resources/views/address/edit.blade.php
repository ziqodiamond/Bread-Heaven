...
<x-layout>
    ...
    <form action="{{ route('address.update', $address) }}" method="POST" x-data="{}" class="m-10">
        ...
        <div class="space-y-6 text-gray-900">
            ...
            <div class="flex items-start justify-between">
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs text-gray-500">Edit Alamat</span>
                    </div>

                    <h1 class="mt-2 text-xl font-semibold text-gray-900">Edit Alamat</h1>
                    <p class="mt-0.5 text-sm text-gray-500">
                        Perbarui detail alamat
                        <span class="font-medium text-gray-700">{{ $address->receiver_name }}</span>
                    </p>
                </div>
            ...
            {{-- ─── Info badge: alamat utama ─── --}}
            @if ($address->is_default)
                <div class="flex items-center gap-2 rounded-xl border border-gray-200 bg-gray-900 px-4 py-3">
                    ...
                    <p class="text-sm font-medium text-white">Ini adalah alamat utama kamu saat ini</p>
                </div>
            @endif
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="text" name="receiver_name"
                                value="{{ old('receiver_name', $address->receiver_name) }}" placeholder="Nama penerima"
                        ...
                        <div>
                            ...
                            <input type="text" name="receiver_phone"
                                value="{{ old('receiver_phone', $address->receiver_phone) }}" placeholder="08xxxxxxxxxx"
                                class="w-full rounded-lg border px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 transition-colors
            ...
            <div class="rounded-xl border border-gray-100 bg-white overflow-hidden">
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="text" name="postal_code"
                                value="{{ old('postal_code', $address->postal_code) }}" placeholder="40135"
                                class="w-full rounded-lg border px-3 py-2.5 text-sm placeholder-gray-400 transition-colors
                    ...
                    <div>
                        ...
                        <textarea name="full_address" rows="3" placeholder="Jl. Contoh No. 123, RT 01/RW 02, Kelurahan..."
                            class="w-full rounded-lg border px-3 py-2.5 text-sm placeholder-gray-400 transition-colors resize-none
                                   {{ $errors->has('full_address') ? 'border-red-300 bg-red-50' : 'border-gray-200 focus:border-gray-400' }}">{{ old('full_address', $address->full_address) }}</textarea>
                        @error('full_address')
                            <p class="mt-1.5 flex items-center gap-1 text-xs text-red-500">
                                <svg class="w-3.5 h-3.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd"
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="2" placeholder="Contoh: Rumah warna biru, pagar hitam..."
                            class="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm placeholder-gray-400 focus:border-gray-400 transition-colors resize-none">{{ old('notes', $address->notes) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">
                            @if ($address->latitude && $address->longitude)
                                Lokasi sudah ter-pin — klik peta untuk memperbarui
                            @else
                    ...
                    <div class="relative rounded-xl overflow-hidden border border-gray-200">
                        ...
                        <div id="map-hint"
                            class="absolute bottom-3 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-sm rounded-lg px-3 py-1.5 text-xs text-gray-600 shadow-sm border border-gray-100 pointer-events-none transition-opacity duration-300
                                   {{ $address->latitude && $address->longitude ? 'opacity-0' : '' }}">
                            <span class="flex items-center gap-1.5">
                                ...
                                Klik pada peta untuk memperbarui lokasi
                            </span>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <div id="lat-indicator"
                                    class="absolute right-3 top-1/2 -translate-y-1/2 {{ $address->latitude ? '' : 'hidden' }}">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <div id="lng-indicator"
                                    class="absolute right-3 top-1/2 -translate-y-1/2 {{ $address->longitude ? '' : 'hidden' }}">
                                    <div class="h-2 w-2 rounded-full bg-green-400 animate-pulse"></div>
                                </div>
                            </div>
                        </div>
                    ...
                    <label
                        ...
                        <div class="relative">
                            ...
                            <input type="checkbox" id="is_default" name="is_default" value="1"
                                class="peer sr-only" {{ old('is_default', $address->is_default) ? 'checked' : '' }}>
            ...
            <div class="rounded-xl border border-red-100 bg-white overflow-hidden" x-data="{ confirmDelete: false }">

                <div class="flex items-center gap-3 border-b border-red-100 px-5 py-4">
                    <div class="flex h-6 w-6 items-center justify-center rounded-md bg-red-50 flex-shrink-0">
                    ...
                    <div>
                        <h2 class="text-sm font-semibold text-red-600">Hapus Alamat</h2>
                ...
                <div class="p-5">
                    <div class="flex items-center justify-between">
                        <div class="min-w-0 mr-4">
                            <p class="text-sm text-gray-600">
                                Hapus alamat ini secara permanen dari akun kamu.
                            </p>
                            @if ($address->is_default)
                                <p class="mt-1 text-xs text-amber-600 flex items-center gap-1">
                                    ...
                                    Ini adalah alamat utama kamu saat ini
                                </p>
                            @endif
                        </div>

                        <button type="button" @click="confirmDelete = true"
                            class="flex-shrink-0 flex items-center gap-2 rounded-lg border border-red-200 px-4 py-2 text-sm text-red-500 hover:bg-red-50 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    {{-- Modal konfirmasi --}}
                    <div x-show="confirmDelete" x-transition:enter="ease-out duration-200"
                        ...
                        <div x-show="confirmDelete" x-transition:enter="ease-out duration-200"
                            ...
                            <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-red-50 mx-auto">
                            ...
                            <p class="mt-1.5 text-center text-sm text-gray-500">
                                ...
                                </span> akan dihapus permanen dan tidak bisa dikembalikan.
                            </p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // ─── Koordinat existing dari model ───
            const EXISTING_LAT = {{ $address->latitude ?? 'null' }};
            const EXISTING_LNG = {{ $address->longitude ?? 'null' }};

            // Jika ada koordinat existing, center di sana; jika tidak, default Bandung
            const centerLat = EXISTING_LAT ?? -6.9175;
            const centerLng = EXISTING_LNG ?? 107.6191;
            const centerZoom = EXISTING_LAT ? 16 : 13;
            ...
            // ─── Load existing marker jika ada ───
            if (EXISTING_LAT && EXISTING_LNG) {
                placeMarker(EXISTING_LAT, EXISTING_LNG);
            }
