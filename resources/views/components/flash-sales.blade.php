                ...
                <div class="flex-shrink-0 w-80">
                    ...
                    <div class="rounded-xl border border-gray-100 bg-white overflow-hidden shadow-md hover:shadow-lg transition-shadow">
                        ...
                        <div class="relative p-4 text-white" style="background: linear-gradient(to right, var(--color-start), var(--color-end)); --color-start: {{ getColorRGB($flashSale->badge_color, 'start') }}; --color-end: {{ getColorRGB($flashSale->badge_color, 'end') }};">
                            ...
                            <div class="mt-3 flex items-center gap-2">
                                @if($flashSale->has_ended)
                                    <span class="inline-block px-3 py-1.5 bg-red-100 text-red-700 rounded-full text-xs font-bold">
                                        ✓ Telah Berakhir
                                    </span>
                        ...
                        <div class="p-4 border-b border-gray-100 bg-gray-50">
                            @if($flashSale->has_ended)
                                ...
                                <div class="text-center py-4">
                                    <p class="text-sm font-bold text-red-600">Flash Sale Telah Berakhir</p>
                                    <p class="text-xs text-gray-500 mt-1">Berakhir: {{ $flashSale->end_at->format('d M Y H:i') }}</p>
                                </div>
                            @elseif($flashSale->show_countdown)
                                @if($flashSale->is_running)
                                    <!-- Countdown to End (Running) -->
                                ...
                                @else
                                    <!-- Countdown to Start (Not Started) -->
                                    <div class="text-center py-4">
                                        <p class="text-xs font-medium text-gray-600 mb-2">Dimulai dalam:</p>
                                        <div class="flex justify-center gap-2">
                                            <div class="flex flex-col items-center">
                                                <div class="bg-white rounded-lg border border-gray-200 px-2 py-1 min-w-[40px]">
                                                    <span class="text-lg font-bold countdown-start-day" data-flashsale-id="{{ $flashSale->id }}" style="color: {{ getColorRGB($flashSale->badge_color, 'text') }};">0</span>
                                                </div>
                                                <span class="text-xs text-gray-500 mt-1">Hari</span>
                                            </div>
                                            <span class="text-lg font-bold text-gray-400">:</span>
                                            <div class="flex flex-col items-center">
                                                <div class="bg-white rounded-lg border border-gray-200 px-2 py-1 min-w-[40px]">
                                                    <span class="text-lg font-bold countdown-start-hour" data-flashsale-id="{{ $flashSale->id }}" style="color: {{ getColorRGB($flashSale->badge_color, 'text') }};">0</span>
                                                </div>
                                                <span class="text-xs text-gray-500 mt-1">Jam</span>
                                            </div>
                                            <span class="text-lg font-bold text-gray-400">:</span>
                                            <div class="flex flex-col items-center">
                                                <div class="bg-white rounded-lg border border-gray-200 px-2 py-1 min-w-[40px]">
                                                    <span class="text-lg font-bold countdown-start-min" data-flashsale-id="{{ $flashSale->id }}" style="color: {{ getColorRGB($flashSale->badge_color, 'text') }};">0</span>
                                                </div>
                                                <span class="text-xs text-gray-500 mt-1">Menit</span>
                                            </div>
                                            <span class="text-lg font-bold text-gray-400">:</span>
                                            <div class="flex flex-col items-center">
                                                <div class="bg-white rounded-lg border border-gray-200 px-2 py-1 min-w-[40px]">
                                                    <span class="text-lg font-bold countdown-start-sec" data-flashsale-id="{{ $flashSale->id }}" style="color: {{ getColorRGB($flashSale->badge_color, 'text') }};">0</span>
                                                </div>
                                                <span class="text-xs text-gray-500 mt-1">Detik</span>
                                            </div>
                                        </div>
                                        <input type="hidden" class="flash-start-time" data-flashsale-id="{{ $flashSale->id }}" value="{{ $flashSale->start_at->timestamp }}">
                                    </div>
                                @endif
                            @else
                                <div class="h-4"></div>
                        ...
                        <div class="p-4 border-t border-gray-100 bg-gray-50">
                            @if($flashSale->has_ended)
                                <button disabled class="block w-full text-center rounded-lg px-4 py-2 text-sm font-bold text-gray-500 bg-gray-300 cursor-not-allowed">
...
<script>
    ...
    function updateCountdowns() {
        ...
        // Countdown to END time (for running sales)
        ...
        // Countdown to START time (for upcoming sales)
        document.querySelectorAll('.flash-start-time').forEach(element => {
            ...
            if (remaining > 0) {
                ...
                document.querySelector(`.countdown-start-day[data-flashsale-id="${flashsaleId}"]`).textContent = String(days).padStart(2, '0');
                document.querySelector(`.countdown-start-hour[data-flashsale-id="${flashsaleId}"]`).textContent = String(hours).padStart(2, '0');
                document.querySelector(`.countdown-start-min[data-flashsale-id="${flashsaleId}"]`).textContent = String(minutes).padStart(2, '0');
                document.querySelector(`.countdown-start-sec[data-flashsale-id="${flashsaleId}"]`).textContent = String(seconds).padStart(2, '0');
            }
        });
    }
